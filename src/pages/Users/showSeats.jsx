...
const MovieTicketBooking = () => {
    ...
    useLayoutEffect(() => {
        ...
        const fetchTheaterScreenData = async () => {
            try {
                ...
                console.log(responseScreen.data.data);
    ...
    const renderSeats = (row, category) => {
        ...
        for (let i = 0; i < row[1].length; i++) {
            const seatId = `${row[0]}${row[1][i].col}`; // Assuming 'col' represents the seat number
            const colValue = row[1][i].col;
            if (colValue !== 0) {
                seats.push(
                    <div className="inline-block mr-4 mt-5" key={seatId}>
                        <div
                            onClick={() => handleSeatSelection(row[0], colValue, i, category)}
                ...
                seats.push(
                    <div className="inline-block mr-4 mt-5" key={seatId}>
                        {/* Render a larger space instead of a button */}
                        <span className="text-xs ml-1 inline-block w-5 h-10"></span>
                    </div>
                );
            }
        }
